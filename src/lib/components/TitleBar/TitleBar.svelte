<script lang="ts">
  ...
  import { X, Minus, Settings, Lock, LockOpen, Check } from "@lucide/svelte";
  import { invoke } from "@tauri-apps/api/core";

  let { windowTitle = $bindable(), usableTitle = $bindable() } = $props();
  ...
  async function findWindow() {
    try {
      const foundTitle: string = await invoke("get_game_title_by_name", { characterName: windowTitle });
...
<div class="flex items-center justify-between bg-stone-800 h-8 relative select-none">
  <div class="absolute inset-0" data-tauri-drag-region></div>

  <div class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-20 flex items-center space-x-2 group">
    <input
      type="text"
      ...
      class="bg-transparent border-none text-md font-bold text-stone-200 text-center
             ...
             {isLocked ? 'pointer-events-none cursor-default' : 'pointer-events-auto cursor-text hover:bg-stone-700/30'}"
      spellcheck="false"
    />
  ...
  <div class="flex flex-row ml-auto h-full relative z-10">
    <Button variant="outline" class="border-none rounded-none h-full w-8 bg-stone-800 hover:bg-stone-700 transition-colors">
      <Settings class="w-4 h-4 text-white" />
    </Button>
    <Button variant="outline" onclick={minimize} class="border-none rounded-none h-full w-8 bg-stone-800 hover:bg-stone-700 transition-colors">
      <Minus class="w-4 h-4 text-white" />
    </Button>
    <Button variant="outline" onclick={close} class="border-none rounded-none h-full w-8 bg-stone-800 hover:bg-red-700 transition-colors">
